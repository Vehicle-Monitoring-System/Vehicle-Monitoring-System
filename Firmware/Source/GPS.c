...
parse_term(gps_t* gh) {
    if (gh->p.term_num == 0) {                  /* Check string type */
        if (0) {
            ...
            gh->p.stat = STAT_GPGSV;
        } else if (!strncmp(gh->p.term_str, "$GLGSV", 6)) {
            gh->p.stat = STAT_GLGSV;
...
#endif /* GPS_CFG_STATEMENT_GPGSA */
#if GPS_CFG_STATEMENT_GPGSV
    } else if (gh->p.stat == STAT_GPGSV) {        /* Process GPGSV statement */
        switch (gh->p.term_num) {
            case 2:                             /* Current GPGSV statement number */
                gh->p.data.gp_gsv.stat_num = (uint8_t)parse_number(gh, NULL);
                break;
            case 3:                             /* Process satellites in view */
                gh->p.data.gp_gsv.sats_in_view = (uint8_t)parse_number(gh, NULL);
                break;
            default:
                ...
                if (gh->p.term_num >= 4 && gh->p.term_num <= 19) {  /* Check current term number */
                    ...
                    index = 4 * (gh->p.data.gp_gsv.stat_num - 1) + term_num / 4;   /* Get array index */
                    if (index < sizeof(gh->gp_sats_in_view_desc) / sizeof(gh->gp_sats_in_view_desc[0])) {
                        value = (uint16_t)parse_number(gh, NULL);   /* Parse number as integer */
                        switch (term_num % 4) {
                            case 0: gh->gp_sats_in_view_desc[index].num = value; break;
                            case 1: gh->gp_sats_in_view_desc[index].elevation = value; break;
                            case 2: gh->gp_sats_in_view_desc[index].azimuth = value; break;
                            case 3: gh->gp_sats_in_view_desc[index].snr = value; break;
                            default: break;
                        }
                    }
                }
    ...
    else if(gh->p.stat == STAT_GLGSV){
        switch (gh->p.term_num) {
            case 2:                             /* Current GPGSV statement number */
                gh->p.data.gl_gsv.stat_num = (uint8_t)parse_number(gh, NULL);
                break;
            case 3:                             /* Process satellites in view */
                gh->p.data.gl_gsv.sats_in_view = (uint8_t)parse_number(gh, NULL);
                break;
            default:
#if GPS_CFG_STATEMENT_GPGSV_SAT_DET
                if (gh->p.term_num >= 4 && gh->p.term_num <= 19) {  /* Check current term number */
                    ...
                    index = 4 * (gh->p.data.gl_gsv.stat_num - 1) + term_num / 4;   /* Get array index */
                    if (index < sizeof(gh->gl_sats_in_view_desc) / sizeof(gh->gl_sats_in_view_desc[0])) {
                        value = (uint16_t)parse_number(gh, NULL);   /* Parse number as integer */
                        switch (term_num % 4) {
                            case 0: gh->gl_sats_in_view_desc[index].num = value; break;
                            case 1: gh->gl_sats_in_view_desc[index].elevation = value; break;
                            case 2: gh->gl_sats_in_view_desc[index].azimuth = value; break;
                            case 3: gh->gl_sats_in_view_desc[index].snr = value; break;
                            default: break;
                        }
                    }
                }
...
#endif /* GPS_CFG_STATEMENT_GPGSA */
#if GPS_CFG_STATEMENT_GPGSV
    } else if (gh->p.stat == STAT_GPGSV) {
        gh->gp_sats_in_view = gh->p.data.gp_gsv.sats_in_view;
    }
      else if (gh->p.stat == STAT_GLGSV) {
        gh->gl_sats_in_view = gh->p.data.gl_gsv.sats_in_view;
#endif /* GPS_CFG_STATEMENT_GPGSV */
#if GPS_CFG_STATEMENT_GPRMC
